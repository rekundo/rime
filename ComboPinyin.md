...
`[R], [O]` 通借爲韻母⟨ei, ou⟩，有一定的適用條件：

 - 這裏討論的韻母⟨ei, ou⟩都不帶介音，即直接拼聲母；
 - 按照聲母表一組一組地觀察；
 - 若一組聲母可同時拼⟨ï⟩與⟨ei⟩，則不得通借 `[R]`；
 - 若一組聲母可同時拼⟨o⟩與⟨ou⟩，則不得通借 `[O]`。

熟悉聲、韻配合規律後，易知：

1. ⟨zh⟩組、⟨z⟩組聲母可同時拼⟨ï⟩與⟨ei⟩，故 `[R]` 不得通借爲⟨ei⟩。 
2. ⟨b⟩組、⟨d⟩組、⟨g⟩組聲母不拼舌尖元音⟨ï⟩而可以拼⟨ei⟩，與這幾組聲母並擊時，`[R]` 通借爲韻母⟨ei⟩。
3. ⟨b⟩組聲母可同時拼⟨o⟩與⟨ou⟩，故 `[O]` 不得通借爲⟨ou⟩。
4. ⟨l⟩雖然可以拼出⟨lo⟩與⟨lou⟩，但⟨lo⟩只表示語氣詞，並且其所屬的⟨d⟩組聲母再無可以與⟨o⟩拼讀者；
   爲了充分利用簡短的並擊碼，本方案特將⟨lo, lou⟩兩個音節合併，從而允許 `[O]` 與⟨d⟩組聲母並擊時適用通借規則。
5. 綜合 3, 4 兩點，可知 `[O]` 與⟨d⟩組、⟨g⟩組、⟨zh⟩組、⟨z⟩組聲母並擊均可表示韻母⟨ou⟩。
6. ⟨j⟩組聲母不能直接拼韻母⟨ei, ou⟩，因此不適用相關通借規則。

列表如下：

|       | 獨用 | ⟨b⟩組 | ⟨d⟩組 | ⟨g⟩組 | ⟨zh⟩組 | ⟨z⟩組 |
|-------|------|-------|-------|-------|--------|-------|
| `[R]` | ⟨er⟩ | ⟨ei⟩  | ⟨ei⟩  | ⟨ei⟩  | ⟨ï⟩    | ⟨ï⟩   |
| `[O]` | ⟨o⟩  | ⟨o⟩   | ⟨ou⟩  | ⟨ou⟩  | ⟨ou⟩   | ⟨ou⟩  |

如果不熟悉聲-韻搭配的情況，韻母⟨ei, ou⟩可以一律並擊 `[RE], [RO]`，在今後的使用中逐漸總結發現可通借的規律，達到簡省擊鍵的目的。
